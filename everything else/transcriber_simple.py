#!/usr/bin/env python3
"""Simplified transcriber with speaker identification (no emotion classification)"""
...
try:
    speaker_identifier.load_model("demo_speaker_identification_model.pkl")
    print("✅ Loaded speaker identification model")
except FileNotFoundError:
    print("⚠️  No speaker identification model found - using generic labels")
...
print("📊 Speaker identification + VAD enabled")
    ...
    while True:
        ...
        if rec.AcceptWaveform(data):
            ...
            if result.get('text', '').strip():
                ...
                # Get speaker info with identification if available
                if speaker_identifier.is_trained:
                    # Use trained model for identification
                    features = speaker_identifier.extract_speaker_features(data)
                    if features:
