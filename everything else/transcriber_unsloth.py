#!/usr/bin/env python3
"""Transcriber with Unsloth-based speaker identification"""
...
# Initialize Unsloth speaker identifier
speaker_identifier = SpeakerIdentifierUnsloth()

# Try to load pre-trained model
try:
    speaker_identifier.load_model("unsloth_speaker_identification_model.pkl")
    print("✅ Loaded Unsloth speaker identification model")
except FileNotFoundError:
    print("⚠️  No Unsloth speaker identification model found - using generic labels")
...
print("📊 Unsloth speaker identification + VAD enabled")
    ...
    while True:
        ...
        if rec.AcceptWaveform(data):
            ...
            if result.get('text', '').strip():
                ...
                # Get speaker info with Unsloth identification if available
                if speaker_identifier.is_trained:
                    # Use trained Unsloth model for identification
                    features = speaker_identifier.extract_voice_features_text(
                        speaker_detector.extract_voice_features(data)
                    )
                    if features:
                        speaker_name, confidence = speaker_identifier.identify_speaker(
                            speaker_detector.extract_voice_features(data)
                        )
