#!/usr/bin/env python3
"""Collect training data for Unsloth-based speaker identification"""
...
class UnslothTrainingDataCollector:
    def __init__(self):
        self.speaker_identifier = SpeakerIdentifierUnsloth()
    ...
    def collect_training_data(self, speakers_config):
        ...
        for speaker_name, config in speakers_config.items():
            ...
            for i in range(config['samples']):
                ...
                # Create filename
                filename = f"training_data_unsloth/{speaker_name}_sample_{i+1}.wav"
                Path("training_data_unsloth").mkdir(exist_ok=True)
...
def main():
    """Main training data collection script for Unsloth"""
    collector = UnslothTrainingDataCollector()
    ...
    print("🎤 Unsloth Speaker Identification Training Data Collection")
    print("=" * 60)
    ...
    # Save training data
    collector.save_training_data(training_data, "unsloth_speaker_training_data.json")
    
    # Train the model
    print("\n🤖 Training Unsloth speaker identification model...")
    trainer = collector.speaker_identifier.train_model(training_data, epochs=2, batch_size=2)
    
    # Save the trained model
    collector.speaker_identifier.save_model("unsloth_speaker_identification_model.pkl")
    
    print(f"\n✅ Training complete!")
    print(f"   Model saved as: unsloth_speaker_identification_model.pkl")
    print(f"   Training data saved as: unsloth_speaker_training_data.json")
