#!/usr/bin/env python3
"""Transcriber with Apple Silicon Unsloth-compatible speaker identification"""
...
# Initialize Apple Silicon Unsloth-compatible speaker identifier
speaker_identifier = SpeakerIdentifierUnslothApple()

# Try to load pre-trained model
try:
    speaker_identifier.load_model("apple_silicon_unsloth_model.pkl")
    print("✅ Loaded Apple Silicon Unsloth-compatible speaker identification model")
except FileNotFoundError:
    print("⚠️  No Apple Silicon Unsloth model found - using generic labels")
...
print("📊 Apple Silicon Unsloth-compatible speaker identification + VAD enabled")
    ...
    while True:
        ...
        if rec.AcceptWaveform(data):
            ...
            if result.get('text', '').strip():
                ...
                # Get speaker info with Apple Silicon Unsloth identification if available
                if speaker_identifier.is_trained:
                    # Use trained Apple Silicon Unsloth model for identification
                    features = speaker_detector.extract_voice_features(data)
                    if features:
                        speaker_name, confidence = speaker_identifier.identify_speaker(features)
                ...
                print(f"📝 {text}")
                print(f"   {speaker_info} | {voice_count}")
                print()
